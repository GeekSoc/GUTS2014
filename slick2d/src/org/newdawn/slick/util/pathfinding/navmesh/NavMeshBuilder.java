...
/**
 * The builder responsible for converting a tile based map into a navigation
 * mesh
 * 
 * @author kevin
 */
public class NavMeshBuilder implements PathFindingContext {
	 ...
	 *            The map to build the navigation mesh from
	 * @param tileBased
	 *            True if we'll use the tiles for the mesh initially rather than
	 *            quad spacing
	 * @return The newly created navigation mesh
	 */
	public NavMesh build(TileBasedMap map, boolean tileBased) {
		...
		if (tileBased) {
			...
			Space space = new Space(0, 0, map.getWidthInTiles(),
	 ...
	 * @return True if a merge occured and we'll have to start the merge process
	 *         again
	 */
	...
	private void subsection(TileBasedMap map, Space space, ArrayList spaces) {
		if (!clear(map, space)) {
			...
			subsection(map, new Space(space.getX(), space.getY(), width2,
					height2), spaces);
			subsection(map, new Space(space.getX(), space.getY() + height2,
					width2, height2), spaces);
			subsection(map, new Space(space.getX() + width2, space.getY(),
					width2, height2), spaces);
			subsection(map, new Space(space.getX() + width2, space.getY()
					+ height2, width2, height2), spaces);
		} else {
			spaces.add(space);
		}
	}
