...
public class Space {
	...
	/**
	 * Link this space to another by creating a link and finding the point at
	 * which the spaces link up
	 * 
	 * @param other
	...
	public void link(Space other) {
		...
		if (inTolerance(x, other.x + other.width)
				|| inTolerance(x + width, other.x)) {
			float linkx = x;
			if (x + width == other.x) {
				linkx = x + width;
			}
		...
		if (inTolerance(y, other.y + other.height)
				|| inTolerance(y + height, other.y)) {
			float linky = y;
			if (y + height == other.y) {
				linky = y + height;
			}
	...
	/**
	 * Check whether two locations are within tolerance distance. This is used
	 * when finding aligned edges to remove float rounding errors
	 * 
	 * @param a
	 *            The first value
	 * @param b
	...
	/**
	 * Check if the given space can be merged with this one. It must have an
	 * adjacent edge and have the same height or width as this space.
	 * 
	 * @param other
	...
	public boolean pickLowestCost(Space target, NavPath path) {
		...
		for (int i = 0; i < getLinkCount(); i++) {
			Link link = getLink(i);
			if ((bestLink == null)
					|| (link.getTarget().getCost() < bestLink.getTarget()
							.getCost())) {
				bestLink = link;
			}
		}
