...
public class InternalTextureLoader {
	...
	public TextureImpl getTexture(InputStream in, String resourceName,
		...
		TextureImpl tex = getTexture(in, resourceName, SGL.GL_TEXTURE_2D,
	...
	private TextureImpl getTexture(InputStream in, String resourceName,
		...
		if (holdTextureData) {
			texture.setTextureData(srcPixelFormat, componentCount, minFilter,
		...
		GL.glTexImage2D(target, 0, dstPixelFormat, get2Fold(width),
				get2Fold(height), 0, srcPixelFormat, SGL.GL_UNSIGNED_BYTE,
				textureBuffer);
	 ...
	 */
	public int reload(TextureImpl texture, int srcPixelFormat,
			int componentCount, int minFilter, int magFilter,
			ByteBuffer textureBuffer) {
		int target = SGL.GL_TEXTURE_2D;
		int textureID = createTextureID();
		GL.glBindTexture(target, textureID);
		...
		// produce a texture from the byte buffer
		GL.glTexImage2D(target, 0, dstPixelFormat, texture.getTextureWidth(),
				texture.getTextureHeight(), 0, srcPixelFormat,
				SGL.GL_UNSIGNED_BYTE, textureBuffer);
