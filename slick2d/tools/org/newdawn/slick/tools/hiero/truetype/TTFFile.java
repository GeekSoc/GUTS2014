...
public class TTFFile {
	...
	/** The table of kerning values for ansi encoding */
	...
	private int numberOfGlyphs; // Number of glyphs in font (read from "maxp"
								// table)
	/** The number of glyphs if using a fixed width font */
	...
	/** True if the font is fixed pitch */
	private long isFixedPitch = 0;
	/** The bounding box top value */
	private int fontBBox1 = 0;
	/** The bounding box left value */
	private int fontBBox2 = 0;
	/** The bounding box bottom value */
	private int fontBBox3 = 0;
	/** The bounding box right value */
	private int fontBBox4 = 0;
	/** The cap height */
	private int capHeight = 0;
	/** The position the underline should be placed at */
	private int underlinePosition = 0;
	/** The thickness of the associated underline */
	private int underlineThickness = 0;
	/** The x height read from the TTF */
	...
	class UnicodeMapping {
		...
		UnicodeMapping(int glyphIndex, int unicodeIndex) {
			...
			glyphToUnicodeMap.put(new Integer(glyphIndex), new Integer(
					unicodeIndex));
			unicodeToGlyphMap.put(new Integer(unicodeIndex), new Integer(
					glyphIndex));
		}
	 ...
	 * 
	 * @param in
	 *            The reader to read the tables from
	 * @param name
	 ...
	 * @throws IOException
	 *             Indicatesa a failure to read
	 */
	boolean seekTab(FontFileReader in, String name, long offset)
			throws IOException {
	 ...
	 * 
	 * @param in
	 *            The reader to get the table from
	 * @return True if the table has been read
	 * @throws IOException
	 *             Indicates a failure to read the table
	 */
						...
						if (cmapRangeOffsets[i] != 0 && j != 65535) {
							...
							if (v != null) {
								...
								while (e.hasNext()) {
									...
									ansiWidth[aIdx.intValue()] = mtxTab[glyphIdx]
											.getWx();
	...
	protected void readHorizontalHeader(FontFileReader in) throws IOException {
		...
		log.debug("hhea.Descender: " + hheaDescender + " " + (hheaDescender));
	...
	protected void readHorizontalMetrics(FontFileReader in) throws IOException {
		...
		for (int i = 0; i < nhmtx; i++) {
			...
			if (TRACE_ENABLED) {
				if (log.isDebugEnabled()) {
					log.debug("   width[" + i + "] = " + (mtxTab[i].getWx())
							+ ";");
				}
			}
		}
	 ...
	 * 
	 * @param in
	 *            The reader to get the names from
	 * @throws IOException
	 *             Indicates a failure to read the table
	 */
		...
		case 0x00020000:
			...
			for (int i = 0; i < l; i++) {
				if (mtxTab[i].getIndex() < NMACGLYPHS) {
					mtxTab[i].setName(Glyphs.MAC_GLYPH_NAMES[mtxTab[i]
							.getIndex()]);
				} else {
	...
	protected final void readIndexToLocation(FontFileReader in)
		...
		for (int i = 0; i < numberOfGlyphs; i++) {
			mtxTab[i].setOffset(locaFormat == 1 ? in.readTTFULong() : (in
					.readTTFUShort() << 1));
		}
	...
	private final void readGlyf(FontFileReader in) throws IOException {
		...
		for (int i = 0; i < numberOfGlyphs; i++) {
			if ((i + 1) >= mtxTab.length
				...
				/**
				 * @todo Verify that this is correct, looks like a copy/paste bug
				 *       (jm)
				 */
	...
	private final boolean readPCLT(FontFileReader in) throws IOException {
		...
		if (dirTab != null) {
			...
			log.debug("xHeight from PCLT: " + xHeight + " " + (xHeight));
			in.skip(2 * 2);
			capHeight = in.readTTFUShort();
			log.debug("capHeight from PCLT: " + capHeight + " " + (capHeight));
		...
		if (dirTab != null) {
			...
			while (ae.hasNext()) {
				...
				while (aee.hasNext()) {
					...
					Iterator uniMap = mtxTab[cidKey2.intValue()]
				...
				if (akpx.size() > 0) {
					Iterator uniMap = mtxTab[cidKey1.intValue()]
							.getUnicodeIndex().listIterator();
					while (uniMap.hasNext()) {
						Integer unicodeKey = (Integer) uniMap.next();
						Integer[] ansiKeys = unicodeToWinAnsi(unicodeKey
	...
	/**
	 * Helper methods, they are not very efficient, but that really doesn't
	 * matter...
	 *
	 * @param unicode
	...
	public void printStuff() {
		...
		if ((italicAngle % 0x10000) > 0) {
			System.out.print("." + (short) ((italicAngle % 0x10000) * 1000)
					/ 0x10000);
		}
		System.out.println();
		System.out.println("Ascender:    " + (ascender));
		System.out.println("Descender:   " + (descender));
		System.out.println("FontBBox:    [" + (fontBBox1) + " " + (fontBBox2)
				+ " " + (fontBBox3) + " " + (fontBBox4) + "]");
	}
	...
	private Integer unicodeToGlyph(int unicodeIndex) throws IOException {
		final Integer result = (Integer) unicodeToGlyphMap.get(new Integer(
				unicodeIndex));
		if (result == null) {
