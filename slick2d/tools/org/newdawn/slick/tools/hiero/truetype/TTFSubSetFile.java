...
public class TTFSubSetFile extends TTFFile {
	 ...
	 *
	 * @param in
	 *            The input to read the CVT from
	 * @throws IOException
	 *             Indicates a failure to read the entry
	 */
		...
		if (entry != null) {
			...
			System.arraycopy(
					in.getBytes((int) entry.getOffset(),
							(int) entry.getLength()), 0, output, currentPos,
					(int) entry.getLength());
	 ...
	 *
	 * @param in
	 *            The stream to read the FPGM table from
	 * @throws IOException
	 ...
	 * 
	 * @param size
	 *            The size of the loca to create
	 * @throws IOException
	 *             Indicate a failure to store the loca
	 */
	 ...
	 *
	 * @param in
	 *            The reader from which to obtain the info
	 * @param size
	 *            The size of the MAXP table to write
	 * @throws IOException
	 *             Indicates a failure to write
	 */
	 ...
	 *
	 * @param in
	 *            The reader to which we're adding
	 * @throws IOException
	 *             Indicates a failure to read the PREP table
	 */
	 ...
	 *
	 * @param in
	 *            The reader from which to grab the HHEA table
	 * @param size
	 *            The size of the table
	 * @throws IOException
	 *             Indicates a failure to read data
	 */
	 ...
	 * 
	 * @param in
	 *            The reader to read the HEAD table from
	 * @throws IOException
	 *             Failure to read the table
	 */
	 ...
	 *
	 * @param in
	 *            Reader to get the table from
	 * @param glyphs
	 *            The glpyhs table to populate
	 * @throws IOException
	 *             Indicates a failure to read data
	 */
		...
		if (entry != null) {
			...
			for (int i = 0; i < origIndexes.length; i++) {
				...
				System.arraycopy(
						in.getBytes((int) entry.getOffset()
								+ (int) mtxTab[origGlyphIndex].getOffset(),
								glyphLength), 0, output, currentPos,
						glyphLength);
	 ...
	 *
	 * @param in
	 *            The reader from which to grab the HMTX table
	 * @param glyphs
	 *            The glyphs table to populate
	 * @throws IOException
	 *             Indicates a failure to read
	 */
	 ...
	 * 
	 * @param in
	 *            The input from which to determine the included glyphs
	 * @param glyphOffset
	 *            The offset the glyph
	 * @param glyphIdx
	 *            The index of the base glyph
	 * @return The list of glyphs building the composite
	 * @throws IOException
	 *             Indicates a failure to read from the font file
	 */
	 ...
	 *             Indicates a failure to read from the font file.
	 */
	private void remapComposite(FontFileReader in, Map glyphs, int glyphOffset,
			Integer glyphIdx) throws IOException {
		int offset = glyphOffset
				+ (int) mtxTab[glyphIdx.intValue()].getOffset() + 10;
	 ...
	 * 
	 * @param in
	 *            The input stream to read the glyphs from
	 * @param glyphs
	 *            The glyphs map to populate
	 * @throws IOException
	 *             Indicates a failure to read from the reader
	 */
			...
			while (newComposites == null || newComposites.size() > 0) {
				...
				while (e.hasNext()) {
					...
					if (in.readTTFShort(entry.getOffset()
						...
						List composites = getIncludedGlyphs(in,
	 ...
	 */
	public byte[] readFont(FontFileReader in, String name, Map glyphs)
			throws IOException {
	 ...
	 *
	 * @param pos
	 *            The position from the file to read
	 ...
	 * 
	 * @param max
	 *            The value to find the maximum power of 2
	 ...
	 *            The start value
	 * @param size
	 *            The size of the values to checksum
