...
public class BMFontUtil {
	...
	public void save(File outputBMFontFile) throws IOException, SlickException {
		...
		out.println("common lineHeight=" + unicodeFont.getLineHeight()
				+ " base=26 scaleW=" + pageWidth + " scaleH=" + pageHeight
				+ " pages=" + unicodeFont.getGlyphPages().size() + " packed=0");

		int pageIndex = 0, glyphCount = 0;
		for (Iterator pageIter = unicodeFont.getGlyphPages().iterator(); pageIter
				.hasNext();) {
		...
		for (Iterator pageIter = unicodeFont.getGlyphPages().iterator(); pageIter
				.hasNext();) {
			GlyphPage page = (GlyphPage) pageIter.next();
			for (Iterator glyphIter = page.getGlyphs().iterator(); glyphIter
					.hasNext();) {
				...
				out.println("char id="
						...
						+ "     width=" + glyph.getWidth() + "     height="
						+ glyph.getHeight() + "     xoffset=" + xOffset
						+ "     yoffset=" + glyph.getYOffset()
						+ "    xadvance=" + xAdvance + "     page=" + pageIndex
						+ "  chnl=0 ");
			}
		...
		if (ttfFileRef == null)
			...
			try {
				kerning.load(ResourceLoader.getResourceAsStream(ttfFileRef),
						font.getSize());
			...
			for (Iterator iter1 = allGlyphs.iterator(); iter1.hasNext();) {
				...
				int firstGlyphCode = getGlyphCode(font,
						firstGlyph.getCodePoint());
				int[] values = kerning.getValues(firstGlyphCode);
				if (values == null)
					continue;
				for (int i = 0; i < values.length; i++) {
					Integer secondCodePoint = (Integer) glyphCodeToCodePoint
							.get(new Integer(values[i] & 0xffff));
					if (secondCodePoint == null)
						continue; // We may not be outputting the second
									// character.
			...
			for (Iterator iter = kernings.iterator(); iter.hasNext();) {
				...
				out.println("kerning first=" + pair.firstCodePoint
						+ "  second=" + pair.secondCodePoint + "  amount="
						+ pair.offset);
			}
		}
		out.close();
		...
		for (Iterator pageIter = unicodeFont.getGlyphPages().iterator(); pageIter
			...
			try {
				imageWriter
						.saveImage(page.getImage(), "png", imageOutput, true);
			} finally {
				imageOutput.close();
			}
			// Flip output image.
			Image image = new ImageIcon(imageOutputFile.getAbsolutePath())
					.getImage();
	...
	private int[] getGlyphMetrics(Font font, int codePoint) {
		// xOffset and xAdvance will be incorrect for unicode characters such as
		// combining marks or non-spacing characters
		// (eg Pnujabi's "\u0A1C\u0A47") that require the context of surrounding
		// glyphs to determine spacing, but thisis the
		// best we can do with the BMFont format.
		char[] chars = Character.toChars(codePoint);
		GlyphVector vector = font.layoutGlyphVector(GlyphPage.renderContext,
				chars, 0, chars.length, Font.LAYOUT_LEFT_TO_RIGHT);
		GlyphMetrics metrics = vector.getGlyphMetrics(0);
		int xOffset = vector.getGlyphPixelBounds(0, null, 0, 0).x
				- unicodeFont.getPaddingLeft();
		int xAdvance = (int) (metrics.getAdvanceX()
				+ unicodeFont.getPaddingAdvanceX()
				+ unicodeFont.getPaddingLeft() + unicodeFont.getPaddingRight());
