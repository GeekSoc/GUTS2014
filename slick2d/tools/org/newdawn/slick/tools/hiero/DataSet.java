...
public class DataSet {
	 ...
	 * 
	 * @param fontName
	 *            The name of the font generated
	 * @param size
	 *            The size of the font generated
	 * @param lineHeight
	 ...
	 *            The name of the image
	 */
	public DataSet(String fontName, int size, int lineHeight, int width,
			int height, String setName, String imageName) {
	...
	public void toAngelCodeText(PrintStream out, String imageName) {
		out.println("info face=\""
				...
				+ setName
				+ "\" unicode=0 stretchH=100 smooth=1 aa=1 padding=0,0,0,0 spacing=1,1");
		out.println("common lineHeight=" + lineHeight + " base=26 scaleW="
				+ width + " scaleH=" + height + " pages=1 packed=0");
		out.println("page id=0 file=\"" + imageName + "\"");

		out.println("chars count=" + chars.size());
		for (int i = 0; i < chars.size(); i++) {
			CharData c = (CharData) chars.get(i);
			out.println("char id=" + c.getID() + "   x=" + c.getX() + "     y="
					+ c.getY() + "     width=" + c.getWidth() + "     height="
					+ c.getHeight() + "     xoffset=0     yoffset="
					+ c.getYOffset() + "    xadvance=" + c.getXAdvance()
					+ "     page=0  chnl=0 ");
		}
		out.println("kernings count=" + kerning.size());
		for (int i = 0; i < kerning.size(); i++) {
			KerningData k = (KerningData) kerning.get(i);
			out.println("kerning first=" + k.first + "  second=" + k.second
					+ "  amount=" + k.offset);
		}
	}
	 ...
	 *            The name of the image
	 * @throws IOException
	 *             Indicates a failure to build the XML
	 */
	public void toAngelCodeXML(PrintStream out, String imageName)
			throws IOException {
		try {
	 ...
	 */
	public void addCharacter(int code, int xadvance, int x, int y, int width,
			int height, int yoffset) {
		chars.add(new CharData(code, xadvance, x, y, width, height, size
				+ yoffset));
	}
	...
	public class CharData {
		 ...
		 *            The offset on the y axis when drawing the character
		 */
		public CharData(int id, int xadvance, int x, int y, int width,
				int height, int yoffset) {
			this.id = id;
