...
public class DistanceFieldFilter {
	 ...
	 */
	private static float separation(final float x1, final float y1,
	 ...
	 * @param scanSize
	 *            The scan size, controls the quality
	 * @param listener
	 *            The lisetener to report progress to
	 * @return The distance field image
	 */
	public static BufferedImage process(BufferedImage inImage, int outWidth,
			int outHeight, int scanSize, ProgressListener listener) {
		System.out.println("DistanceFieldFilter.process");

		BufferedImage outImage = new BufferedImage(outWidth, outHeight,
				BufferedImage.TYPE_4BYTE_ABGR);
		float[][] distances = new float[outImage.getWidth()][outImage
				.getHeight()];
		...
		for (int x = 0; x < outImage.getWidth(); x++) {
			listener.reportProgress("Finding signed distance", x,
					outImage.getWidth());
			for (int y = 0; y < outImage.getHeight(); y++) {
				distances[x][y] = findSignedDistance((x * blockWidth)
						+ (blockWidth / 2), (y * blockHeight)
						+ (blockHeight / 2), inImage, scanSize, scanSize);
			}
		}
	 ...
	 * @param inImage
	 *            The image on which the point exists
	 * @param scanWidth
	 *            The scan line of the image
	 * @param scanHeight
	 *            The scan height of the image
	 * @return The signed distance
	 */
	private static float findSignedDistance(final int pointX, final int pointY,
			BufferedImage inImage, final int scanWidth, final int scanHeight) {
