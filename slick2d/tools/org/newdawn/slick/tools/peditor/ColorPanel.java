...
public class ColorPanel extends ControlPanel {
	...
	public ColorPanel() {
		...
		yPos += 70;
		startAlpha = new ValuePanel("Starting Alpha", 0, 255, 255,
				"The alpha value for particles at their birth", false);
		addValue("startAlpha", startAlpha);
		endAlpha = new ValuePanel("Ending Alpha", 0, 255, 0,
				"The alpha value for particles at their death", false);
		addValue("endAlpha", endAlpha);

		// rendering panel
		JPanel renderingPrimitivePanel = new DefaultPanel();
		renderingPrimitivePanel.setLayout(new BoxLayout(
				renderingPrimitivePanel, BoxLayout.X_AXIS));
		...
		renderingTypePanel.setLayout(new BoxLayout(renderingTypePanel,
				BoxLayout.X_AXIS));
	...
	private void updateColors() {
		...
		for (int i = 0; i < grad.getControlPointCount(); i++) {
			...
			Color slick = new Color(col.getRed() / 255.0f,
					col.getGreen() / 255.0f, col.getBlue() / 255.0f, 1.0f);
	...
	protected void linkEmitterToFields(ConfigurableEmitter emitter) {
		...
		Color end = ((ColorRecord) emitter.colors
				.get(emitter.colors.size() - 1)).col;
	...
	/**
	 * Get the panel controlling the alpha value of particles at the end of
	 * their life
	 * 
	...
	/**
	 * Get the panel controlling the alpha value of particles at the start of
	 * their life
	 * 
	 * @return The panel controlling the alpha value
	 */
