...
public class SheetPanel extends JPanel {
	 ...
	 * 
	 * @param p
	 *            The GUI tool this panel is part of
	 */
	public SheetPanel(Packer p) {
		...
		g.fillRect(image.getWidth() / 2, 0, image.getWidth() / 2,
				image.getHeight() / 2);
		g.fillRect(0, image.getHeight() / 2, image.getWidth() / 2,
				image.getHeight() / 2);

		background = new TexturePaint(image, new Rectangle(0, 0,
	...
	public void paint(Graphics g1d) {
		...
		for (int i = 0; i < selected.size(); i++) {
			...
			g.drawRect(sprite.getX(), sprite.getY(), sprite.getWidth(),
					sprite.getHeight());
		}
	}
}
