...
public class Workspace extends WorkerSpace {
	...
	public void update(GameContainer cont, StateBasedGame game, int delta) {
		...
		if (cont.getInput().isMousePressed(Input.MOUSE_LEFT_BUTTON)) {
			...
			if (menu2.contains(mouseX, mouseY)) {
				GameTime.setSpeed(0);
				game.enterState(0);
			}
		}
	...
	public void render(GameContainer container, StateBasedGame game, Graphics g) {
		...
		menu.draw(container.getWidth()/2, 60, 20, 20);
		menu2 = new Rectangle(container.getWidth()/2, 60, 20, 20);
	...
	public float getTotalWages() {
		...
		return (t+super.getTotalWages())*60;
	}
